...
class OuterPlusDialogActivity: Activity(){
            ...
            if(binding.etTopRealName.text.toString() == "")
                ...
                runBlocking {
                    if (fromDetail == "true") {//detailDialog에서 실행한 경우
                        clothDAO.deleteOuterCloth(clothDAO.getOuterByName(intent.getStringExtra("name")!!))
                        setResult(RESULT_OK, Intent())
                    }
                    clothDAO.insertOuterCloth(outerCloth)
                }
                finish()
            }
        }
